...
for i in "${!SOURCE_DIRS[@]}"; do
  ...
  "cd /app/source && chmod +x id_statistics_generator_8.sh && SKIP_COVERAGE=$SKIP_COVERAGE ./id_statistics_generator_8.sh \"$MODULE\" \"$FULL_TEST_NAME\" \"$ITERATIONS\" \"$NONDEXSEED\""
